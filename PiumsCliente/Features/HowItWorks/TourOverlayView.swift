...
struct TourOverlayView: View {
            ...
            GeometryReader { geo in
                ZStack(alignment: .bottom) {
                    ...
                    VStack(spacing: 0) {
                        ...
                        .padding(.bottom, -1)
                        .animation(.spring(response: 0.4, dampingFraction: 0.8), value: step.tab)
                        ...
                        VStack(spacing: 0) {
                            ...
                            // Contenido animado con slide direccional
    ...
    @ViewBuilder
    private func stepContent(step: TutorialManager.TourStep) -> some View {
    ...
    private var slideTransition: AnyTransition {
        let edge: Edge = tutorial.stepDirection == .forward ? .trailing : .leading
        let reverseEdge: Edge = tutorial.stepDirection == .forward ? .leading : .trailing
        return .asymmetric(
            insertion: .move(edge: edge).combined(with: .opacity),
            removal:   .move(edge: reverseEdge).combined(with: .opacity)
        )
    }
